...
$('#customers').on('changed.bs.select', function () {
    if (syncing) return;
    syncing = true;
...
$('#customerPhone').on('input', function () {
    ...
    if (customerByPhone.has(phone)) {
        ...
        syncing = true;
        $('#customers').selectpicker('val', c.phone).trigger('changed.bs.select');
...
if (orderStatus.length) {
    select2Focus(orderStatus);
    orderStatus.wrap('<div class="position-relative"></div>').select2({
    placeholder: 'Select value',
...
$('#createOrder').on('shown.bs.offcanvas', function () {
    // 폼 초기화
    originalCustomerName = null;
    selectedCustomerPhone = null;
...
btnSubmit.addEventListener("click", () => {
    ...
    const currentName = $('#customerName').val().trim();
    let confirmText = '';
